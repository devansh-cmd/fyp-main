...
Several recent studies have advanced the state of the art on monolingual PD speech corpora. Aversano et al. trained CNN and LSTM models on mel-spectrograms extracted from sustained vowels and read phrases, achieving a macro F1-score of 0.970 on the Italian voice dataset~\cite{aversano2024}. Bandini et al. applied pre-trained CNNs (VGG, ResNet) with transfer learning to spectrogram images of sustained phonations, reporting an F1-score of 0.987 and an average accuracy of 98.65\%~\cite{bandini2023}. On the Colombian Spanish PC-GITA dataset, Orozco-Arroyave et al. applied CNNs to diadochokinetic (DDK) tasks and achieved strong detection rates~\cite{orozco2014}. Despite these advances, three significant limitations persist in the literature:
...
The shift to spectrogram-based deep learning enabled end-to-end feature learning. Bandini et al.~\cite{bandini2023} applied transfer learning with pre-trained CNNs (VGG~\cite{simonyan2015}, ResNet~\cite{he2016}) to spectrogram images of sustained Italian vowels, achieving an F1-score of 0.987. Aversano et al.~\cite{aversano2024} trained CNN and LSTM models on mel-spectrograms of Italian vowels and read phrases, reporting a macro F1 of 0.970. On the Colombian Spanish PC-GITA corpus, Orozco-Arroyave et al.~\cite{orozco2014,orozco2016} analysed diadochokinetic (DDK) tasks using both classical and deep features. These studies establish strong baselines but operate exclusively within a single language, making it difficult to assess whether learned representations capture language-independent pathological markers or merely language-specific articulatory patterns.
...
Multi-branch architectures combine complementary feature extractors to enrich the learned representation. Early dual-stream networks fused hand-crafted and learned features~\cite{orozco2016}. More recently, deep multi-branch models have combined multiple CNN backbones trained on different augmentation strategies~[CITATION NEEDED]. Zhao et al. proposed a two-stream spectrogram network achieving state-of-the-art results on environmental sound classification~[CITATION NEEDED], while Pham et al. explored ensemble-like fusion of EfficientNet variants for audio tagging~[CITATION NEEDED].
...
During training, we apply a stochastic augmentation pipeline to the waveform before spectrogram extraction: (i) additive Gaussian noise at a random SNR in $[20, 40]$\,dB, (ii) pitch shift uniformly drawn from $[-2, +2]$ semitones, (iii) time stretching with rate $r \in [0.8, 1.2]$, (iv) simulated room impulse response (reverb) using randomly sampled RIRs~[CITATION NEEDED], and (v) random gain in $[0.5, 1.5]$. Each augmentation is applied independently with probability 0.5, increasing effective training-set diversity.
...
[TODO: FIGURE \ref{fig:cross_dataset}]
...
\begin{table}[t]
  ...
  \begin{tabular}{llc}
    ...
    \multirow{3}{*}{Italian PD}
      ...
      & Aversano et al.~\cite{aversano2024}$^\dagger$      & 0.970 \\
      & Bandini et al.~\cite{bandini2023}$^\dagger$        & 0.987 \\
...
On the Italian PD corpus, our proposed model achieves a mean macro F1 of 0.964, compared with 0.970 reported by Aversano et al.~\cite{aversano2024} and 0.987 reported by Bandini et al.~\cite{bandini2023}. The remaining gap is less than one percentage point.

A direct comparison requires an important methodological caveat. Neither Aversano et al.~\cite{aversano2024} nor Bandini et al.~\cite{bandini2023} report explicit speaker-identity controls in their cross-validation procedures. In the Italian PD dataset, multiple recordings per speaker are present; without speaker-level grouping, test sets may include recordings from speakers also present in training, inflating reported metrics~\cite{sakar2013}. Our results are obtained under StratifiedGroupKFold with a verified zero-leakage guarantee (\texttt{RuntimeError} on any subject overlap). The 0.006 gap to Aversano et al.~\cite{aversano2024} should therefore be interpreted in this context: our model achieves near-state-of-the-art performance under strictly more conservative evaluation conditions. On PC-GITA, ESC-50, and EmoDB, the proposed model outperforms all available published baselines.
...
[TODO: FIGURE \ref{fig:fold3_anomaly}]
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/fig_cross_dataset.pdf}
    \caption{Cross-dataset evaluation (15-run means) across four distinct speech corpora. The proposed DualCNN-SA-LSTM consistently outperforms the ResNet-50 baseline and intermediate models across all evaluate languages and tasks.}
    \label{fig:cross_dataset}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/fig_fold3_anomaly.pdf}
...
L.~Aversano, M.~L. Bernardi, M.~Cimitile, and R.~Pecori, ``Deep neural networks for Parkinson's disease detection from mel-spectrogram representations of vowels and phrases,'' in \textit{Proc. IEEE Int. Joint Conf. Neural Netw. (IJCNN)}, 2024.

\bibitem{bandini2023}
A.~Bandini, S.~Orlandi, H.~J. Escalante, and F.~Giovannelli, ``Transfer learning with pre-trained deep convolutional neural networks for the automatic assessment of Parkinson's disease from sustained phonations,'' \textit{Biomed. Signal Process. Control}, 2023.
...
K.~Simonyan and A.~Zisserman, ``Very deep convolutional networks for large-scale image recognition,'' in \textit{Proc. Int. Conf. Learn. Representations (ICLR)}, 2015.
