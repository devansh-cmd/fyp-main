...
This report has been prepared on the basis of my own work. Where other published and unpublished source materials have been used, these have been acknowledged. Where AI tools (Claude Code, Antigravity IDE) were used for code execution and writing assistance, this is declared in Appendix~D. All architectural designs, experimental decisions, and research findings are my own original work conducted under the supervision of Dr.\ Li Zhang.
...
SOTA comparison & None & Within 0.7\% of published SOTA under stricter evaluation \\
\bottomrule
\end{tabularx}
\end{table}
...
Aversano et al.~\cite{aversano2024} report F1\,=\,0.971 on the Italian PD dataset using a machine learning pipeline with acoustic feature extraction. Published results on this corpus vary in the evaluation conditions used, which makes cross-study comparison difficult. This thesis addresses that gap by evaluating under a fully documented, speaker-independent protocol with certified zero leakage, producing results that are directly reproducible and verifiable.
...
6 & Mar 2026 & Final SOTA push with FP-SA and label smoothing & 0.964 $\pm$ 0.051; within 1 SD of SOTA (0.971) \\
\bottomrule
\end{tabularx}
\end{table}
...
All three models exhibited consistent performance degradation on Fold~3 of the Italian PD dataset. Table~\ref{tab:fold3_phase2} shows the fold-level breakdown.
...
\begin{figure}[htbp]
    ...
    \caption{Phase 2 fold-level F1 scores on the Italian PD dataset across three architectures. Fold 3 exhibits a consistent performance anomaly (highlighted), indicating a cluster of borderline healthy control subjects.}
    \label{fig:fold3_anomaly}
\end{figure}

The consistency of this degradation across three fundamentally different architectures (single CNN, lightweight CNN, dual-branch fusion) confirms that the difficulty is a property of the data partition, not an architectural weakness. Zero data leakage was confirmed. Investigation revealed that Fold~3's validation set contains approximately 21 healthy control subjects whose speech spectrograms consistently resemble PD patterns, where HC recall drops to approximately 75\% while PD recall remains 1.000. Single-split evaluation, which is standard in the literature, would have made this fold-level instability entirely invisible.
...
+ EfficientNetV2-S branch & 0.964 $\pm$ 0.051 & $+0.6\%$ \\
Full \dualcnn{} & 0.964 $\pm$ 0.051 & confirmed \\
\bottomrule
\end{tabular}
\end{table}
...
The variance reduction from baseline (0.049) to CA (0.041) is notable: CA stabilises performance by providing a learnable mechanism to weight the most diagnostically informative spatial regions, reducing fold-to-fold sensitivity. The variance increase with BiLSTM (0.058) reflects the LSTM's sensitivity to sequence-level patterns that vary more across folds than spatial features.
...
\begin{equation}
    \text{F1}_{\texttt{dual\_cnn\_sa\_lstm}} = 0.964 \pm 0.051 \quad (N = 15)
\end{equation}
...
To rigorously validate the architectural improvements, the final \dualcnn{} model ($0.964 \pm 0.051$) was compared against the primary Phase~2 ResNet50 baseline ($0.926 \pm 0.046$). A Wilcoxon signed-rank test performed on the $N\!=\!15$ paired fold observations confirms that the consistent performance gain across folds is statistically significant ($p < 0.05$). This non-parametric confirmation substantiates the efficacy of the dual-branch and frequency-prior attention mechanisms.
...
\midrule
Italian PD & \dualcnn{} & 0.964 $\pm$ 0.051 & Primary clinical, $N\!=\!15$ \\
...
The proposed system achieves F1\,=\,$0.964 \pm 0.051$ ($N\!=\!15$) on the Italian PD dataset under speaker-independent 5-fold cross-validation with certified zero leakage. To the author's knowledge, this is the strongest result reported on this dataset under a fully verifiable evaluation protocol. Aversano et al.~\cite{aversano2024} report F1\,=\,0.971 under evaluation conditions that are not fully specified in their paper; the gap of 0.007 falls well within this work's standard deviation of $\pm 0.051$, so the results are not statistically distinguishable. Given that this work applies stricter evaluation conditions, the two results are best interpreted as equivalent performance, with this work providing the stronger reproducibility guarantee.
...
One unresolved internal validity concern is the hand-designed FP-SA prior: while the frequency bias improves median F1, a fully controlled ablation between hand-designed and data-driven priors has not been conducted. The frequency prior represents a design bias that is not fully ablated. This is acknowledged as a limitation rather than concealed.

\subsection{External Validity}

External validity concerns whether results generalise beyond this study. The Italian PD corpus is limited to Italian speakers performing read speech tasks. PC-GITA covers Colombian Spanish DDK tasks, a different language and fundamentally different speech task. The cross-lingual gap (0.964 $\to$ 0.780) demonstrates that generalisation is non-trivial and must not be assumed across linguistic or demographic groups. Both datasets are small by deep learning standards ($\sim$65 and 101 subjects respectively), further limiting external validity claims. Any deployment beyond the evaluated populations would require prospective validation.
...
$N\!=\!15$ per dataset (3 seeds $\times$ 5 folds) is sufficient for Wilcoxon signed-rank testing on 5-fold paired comparisons. Non-parametric testing was chosen because normality cannot be assumed with $N\!=\!5$ fold observations. Standard deviations reported reflect genuine fold-to-fold variance, not noise. The Fold~3 anomaly inflates Italian PD variance ($\text{std}\!=\!0.051$ vs $\text{std}\!=\!0.006$ for EmoDB). This is a data property reported transparently, not suppressed.
...
The dual leakage discovery, the Strategy~A$\to$B confound correction, and the six-phase architectural progression all reflect the same principle: identifying and correcting methodological problems is a normal part of research, and reporting them honestly is more useful to the field than hiding them.

In summary, the work makes five contributions: a novel frequency-aware attention mechanism for clinical spectrograms; a certified zero-leakage evaluation pipeline; a cross-lingual PD detection evaluation under unified conditions; the identification of a borderline HC cluster not previously documented; and a documented architectural progression from baseline to a SOTA-competitive system. The results show that careful evaluation methodology and targeted architectural choices can reach competitive performance on clinical speech detection while being transparent about where and why the system falls short.

% ═══════════════════════════════════════════════════════════════
% CHAPTER 10 — PROFESSIONAL ISSUES
% ═══════════════════════════════════════════════════════════════
\chapter{Professional Issues: Ethics and Safety of AI in Clinical Diagnosis}

\section{The Clinical Stakes}

Applying AI to clinical diagnosis carries real consequences beyond the metrics reported in research papers. In PD speech detection specifically, the two error types are not symmetric. A false negative, failing to flag a PD patient, denies that individual early intervention at a stage when therapeutic outcomes are most favourable. A false positive, incorrectly identifying a healthy individual as having PD, inflicts psychological distress, triggers unnecessary and potentially invasive follow-up procedures, and imposes economic burden on both the individual and the healthcare system.

The BCS Code of Conduct mandates that computing professionals ``have due regard for the public health, privacy, security and wellbeing of others and the environment.'' The ACM Code of Ethics similarly requires that ``computing professionals should take action not to discriminate'' and that systems should ``minimise the possibility of indirectly harming others.'' These professional obligations are not abstract principles; they impose concrete requirements on how clinical AI systems are developed, evaluated, and deployed.

This project's evaluation framework was designed with these obligations in mind. The choice to report Control Recall alongside Macro F1 directly addresses the asymmetric cost of false positives. The systematic fold-level analysis that revealed the Fold~3 anomaly was not a standard evaluation requirement; it was conducted specifically because subgroup-level performance failures in clinical AI can cause direct harm to identifiable patient populations.

\section{Data Privacy and Consent}

Patient speech recordings are biometric data that carries speaker identity information that persists even after diagnostic labels are removed. The Italian PD corpus~\cite{dimauro2019} and PC-GITA corpus~\cite{orozco2014} were collected under institutional ethical review with informed consent for research use. This project operates strictly within the scope of that consent: research-only use, no re-identification attempts, no distribution beyond the research context.

GDPR Article~22 establishes the right not to be subject to decisions based solely on automated processing, including profiling. Any deployment of this system in a clinical setting within the EU would require: explicit informed consent from each patient; transparency about how the AI system reaches its decisions; the right to human review of any AI-generated assessment; and data protection impact assessment under Articles 35--36.

This project implements two specific privacy safeguards. First, all subject identifiers in the dataset are anonymised numerical codes; the pipeline never processes or stores any personally identifiable information. Second, SHA-256 cryptographic hashes in spectrogram filenames provide traceability without re-identification, allowing a researcher to verify that a specific spectrogram originated from a specific anonymised recording without learning anything about the patient's identity.

\section{Bias, Fairness, and the Fold~3 Implication}

The Fold~3 anomaly exposes a concrete fairness risk. The approximately 21 HC subjects whose spectrograms are consistently classified as PD represent an identifiable subgroup for whom the system performs substantially worse than its aggregate metrics suggest. If this system were deployed without disclosure of this known failure mode, these individuals would receive false positive PD screenings at a rate approximately four times higher than the general healthy population.

Deploying a system with known differential performance across subgroups raises serious questions. Under both BCS and ACM professional codes, known failure modes must be disclosed to all relevant stakeholders, including clinicians, patients, and regulatory bodies, before deployment. Deploying this system without disclosing the Fold~3 finding would conflict with the professional obligation to ``identify and minimise the impact of potential ethical issues.''

The cross-lingual performance gap (Italian: 0.964; Colombian Spanish: 0.780) raises a related concern. A system trained primarily on Italian speakers and deployed in a multilingual clinical setting would provide substantially inferior screening quality for non-Italian speakers. Given that Parkinson's Disease prevalence does not correlate with native language, this performance gap constitutes a form of algorithmic bias with direct clinical consequences. Ethical deployment requires explicit characterisation of the linguistic populations for which the system has been validated, and clear prohibition of use on populations outside this validated scope.

\section{Reflection on Professional Practice}

Two episodes during this project illustrate the practical application of professional integrity in research.

The dual leakage discovery demanded a choice: report the inflated results (which would have been indistinguishable from legitimate findings in a published paper) or acknowledge the error, discard the affected results, and implement corrections. The decision to discard and correct was clear, though it meant approximately two weeks of experimental work was lost. Discovering the leakage through systematic internal verification rather than an external audit reinforced the importance of building verification into the research process from the start, rather than as a last step.

Similarly, correcting the Aversano citation (fixing the year from 2024 to 2022, the reported F1 from 0.970 to 0.971, and verifying the venue and author list) and removing the unverified Bandini citation were straightforward corrections with no benefit beyond accuracy. Every claim in a research document, including claims about other researchers' work, should be verified before submission.

The use of AI tools in this project (Claude Code for code execution and thesis writing assistance, Antigravity IDE for development) is disclosed in Appendix~D. These tools were used as productivity aids; all architectural designs, experimental decisions, and research interpretations are the author's original work.

% ═══════════════════════════════════════════════════════════════
% CHAPTER 11 — PLANNING AND SCHEDULE
% ═══════════════════════════════════════════════════════════════
...
Generative AI tools (Claude Code and Antigravity IDE) were used solely for supportive purposes, including:
\begin{itemize}
