...
Parkinson's Disease (PD) affects over ten million people worldwide, and speech changes are among the earliest detectable signs of the condition. Most deep learning approaches to PD speech detection are evaluated on a single dataset, do not control for data leakage, and make no attempt to test cross-lingual generalisation. This thesis presents the \dualcnn{} architecture, which combines dual-branch convolutional neural networks (ResNet50 and EfficientNetV2-S) with a custom \fpsa{} (FP-SA) module, Coordinate Attention, and a Bidirectional LSTM temporal head for binary PD versus healthy control classification from log-Mel spectrograms. All experiments use speaker-independent five-fold cross-validation with a certified zero-leakage guard, giving $N\!=\!15$ observations per dataset (three seeds $\times$ five folds), with Wilcoxon signed-rank testing for statistical comparisons.

On the Italian PD dataset, the system achieves a mean macro F1 of $0.964 \pm 0.048$ ($N\!=\!15$). To the best of the author's knowledge, this is the strongest result reported on this dataset under a fully documented, reproducible evaluation protocol. Cross-lingual evaluation on the Colombian Spanish PC-GITA diadochokinetic corpus gives F1\,=\,0.780, which quantifies the generalisation gap between the two languages and speech tasks. A fold-level breakdown reveals a persistent drop on Fold~3 of the Italian PD dataset, consistent across all architectures and all three random seeds, traced to roughly 21 healthy control subjects whose spectrograms consistently resemble those of PD patients. This subgroup does not appear to have been identified in prior work on this dataset.

The main contributions are: (1)~the \dualcnn{} architecture with the FP-SA frequency-prior attention mechanism; (2)~a zero-leakage evaluation pipeline with an active RuntimeError guard; (3)~cross-lingual PD evaluation across Italian and Colombian Spanish under identical experimental conditions; (4)~identification and characterisation of the Fold~3 borderline healthy control cluster; and (5)~a six-phase experimental progression documenting how each architectural decision was justified by empirical evidence from the preceding phase.
...
Aversano et al.~\cite{aversano2022} report F1\,=\,0.971 on the Italian PD dataset using a machine learning pipeline with acoustic feature extraction, but do not disclose whether speaker-independent splits were used or whether any data leakage controls were applied. Without knowing the splitting strategy, it is unclear whether their reported figure reflects genuine generalisation or benefits from speaker-level leakage. This thesis addresses that gap by evaluating under a fully documented, speaker-independent protocol with certified zero leakage, producing results that are directly reproducible and verifiable.

Rusz et al.~\cite{rusz2011} demonstrated that quantitative acoustic measurements, particularly in formant transitions, voice onset time, and speech rate, can characterise early untreated PD with clinical sensitivity. Their work establishes the acoustic basis for the FP-SA frequency prior: pathological features cluster at specific frequency bands rather than being uniformly distributed, justifying a non-isotropic attention mechanism. The FP-SA hand-crafted frequency bias addresses this gap directly: it concentrates attention weight in the 250--2000\,Hz sub-band where PD-associated formant disruption is most pronounced. The absence of subject-independent evaluation in Sakar et al.\ and Aversano et al.\ also informed the decision to adopt a fully certified, speaker-independent five-fold protocol with a runtime leakage guard, so that the performance claims in this thesis are reproducible and directly comparable to future work.
...
Coordinate Attention~\cite{hou2021} addresses the limitations of both SE and CBAM by decomposing attention into direction-aware components without collapsing spatial dimensions entirely. Its factorised design, with horizontal pooling for the time-axis and vertical pooling for the frequency-axis, maps naturally onto the axes of a spectrogram, providing the directional specificity that SE lacks and the stability that CBAM fails to deliver. Phase~4 ablation confirmed CA as the strongest single-module attention mechanism for clinical speech data, motivating its inclusion in the final architecture. This is why CA is placed in parallel with FP-SA rather than in sequence: the sequential channel-then-spatial pipeline that destabilises CBAM was avoided by routing the two attention branches independently and fusing via element-wise addition. The $\pm 19.3\%$ F1 variance on EmoDB provided a concrete reference point; the FP-SA variance of $\pm 2.1\%$ across seeds on Italian PD confirms the parallel design avoids the instability that sequential attention produces on spectrogram inputs.
...
A critical consideration is normalisation: computing training-set mean and standard deviation for input normalisation, as is standard practice, introduces a subtle form of data leakage when the same statistics are applied at inference. This work eliminates this risk by using fixed ImageNet normalisation parameters ($\mu = [0.485, 0.456, 0.406]$, $\sigma = [0.229, 0.224, 0.225]$) throughout, ensuring that no training-set statistics propagate to the validation forward pass. This motivated the dual-backbone design: ResNet-50 carries deep ImageNet texture representations while EfficientNetV2-S captures finer-grained spectral patterns through compound scaling, and their fusion covers complementary aspects of transfer that a single-backbone approach misses. The gap between classical feature pipelines (Dimauro et al.) and deep learning on the Italian PD corpus also justified the choice of deep transfer learning as the primary experimental vehicle rather than a hybrid of handcrafted and learned features.
...
No prior work, to the author's knowledge, has evaluated a single deep learning pipeline on both the Italian PD corpus and the PC-GITA DDK corpus under identical experimental conditions (same architecture, same hyperparameters, same evaluation protocol). This thesis fills that gap, providing the first direct measurement of cross-lingual generalisation for a spectrogram-based PD detection system. Cross-lingual generalisation requires capturing speech rhythm and articulation dynamics that vary across languages, which is why a BiLSTM temporal head operating over time-ordered spectrogram frame embeddings was preferred over a purely spatial classifier. The measured gap (Italian PD F1\,=\,0.964 to PC-GITA F1\,=\,0.780) is also why all performance claims in this thesis are scoped to the evaluated language and task rather than presented as general PD detection results.
...
Both bugs were caught and corrected before any final results were generated. The experience shows that even with careful experimental design, leakage can be introduced through non-obvious mechanisms, which is precisely why active verification is necessary rather than relying on split descriptions alone. These two bugs shaped the three-level certification framework: Level~1 (grouped splitting by subject ID), Level~2 (independent offline audit via \texttt{verify\_leakage.py}), and Level~3 (active RuntimeError guard in \texttt{train\_unified.py}). The Kaggle path-resolution bug specifically required Level~3, because it was environment-dependent and would not have been caught by static split inspection; the runtime guard catches leakage introduced at execution time regardless of how the environment resolves file paths.
...
+ Coordinate Attention (CA) & 0.957 $\pm$ 0.041 & $+0.7\%$ \\
+ BiLSTM temporal head & 0.958 $\pm$ 0.058 & $+0.1\%$ \\
+ EfficientNetV2-S branch + FP-SA\textsuperscript{\dag} & 0.964 $\pm$ 0.048 & $+0.6\%$ \\
Full \dualcnn{} & 0.964 $\pm$ 0.048 & confirmed \\
\bottomrule
\end{tabular}
\vspace{0.3em}
{\small \textsuperscript{\dag}FP-SA and CA run in parallel; this row adds both the EfficientNetV2-S branch and FP-SA together. An isolated ablation of FP-SA alone was not conducted and is acknowledged as a limitation (Section~\ref{sec:validity}).}
\end{table}
...
Aversano et al.~\cite{aversano2022} & Italian PD & F1\,=\,0.971 & NR & NR & NR & \textbf{No} \\
...
Grad-CAM visualisation of the misclassified HC spectrograms can disambiguate between the two hypotheses by identifying which spectral regions drive the PD-like classifications. Section~\ref{sec:gradcam} below presents this analysis.

\subsection{Grad-CAM Spectral Analysis of Fold~3 Misclassifications}
\label{sec:gradcam}

Grad-CAM visualisation was applied to the ResNet-50 model trained on Fold~3 (speaker-independent, zero-leakage) to identify which spectral regions drive the systematic false-positive pattern. Inference was run across all HC subjects in the Fold~3 validation set ($n_{\text{HC}}=86$), yielding 22 subjects predicted as PD (false positives, 25.6\%), consistent with the confusion matrix results reported in Table~7.5. Grad-CAM heatmaps were computed for three subject groups: misclassified HC (HC predicted as PD), correctly classified PD, and correctly classified HC, using the final convolutional layer of ResNet-50 layer4 as the target layer.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\linewidth]{figures/gradcam/gradcam_fold3_comparison.png}
    \caption{Grad-CAM overlay comparison for Italian PD Fold~3 (ResNet-50, speaker-independent validation set). Top row: log-Mel spectrograms ($224\times224$, 128 Mel bands). Bottom row: Grad-CAM overlay (jet colourmap blended at 45\%). Red rectangles mark the FP-SA frequency prior band (250--2000\,Hz, Mel bands 20--80). Columns 1--3: HC subjects misclassified as PD (false positives). Columns 4--5: PD subjects correctly classified. Columns 6--7: HC subjects correctly classified.}
    \label{fig:gradcam_comparison}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/gradcam/gradcam_fold3_freq_bands.png}
    \caption{Mean Grad-CAM activation per frequency band for three subject categories in Italian PD Fold~3 (ResNet-50). Shaded region marks the FP-SA frequency prior band (250\,Hz--2\,kHz). Misclassified HC subjects show activation concentrated in the 250--800\,Hz (F1/F2 formant) and 800\,Hz--2\,kHz (F3/voice quality) bands, overlapping substantially with the PD activation profile. Correctly classified HC subjects show comparatively higher activation in the 2--8\,kHz range.}
    \label{fig:gradcam_freqbands}
\end{figure}

Figure~\ref{fig:gradcam_freqbands} shows a consistent pattern across the 22 Fold~3 false positives. Misclassified HC subjects have high Grad-CAM activation in the 250--800\,Hz formant region and the 800\,Hz--2\,kHz voice quality band --- the frequency range where PD-associated formant disruption and reduced articulatory precision are clinically documented~\cite{rusz2011}. Correctly classified HC subjects show stronger activation in the 2--8\,kHz range, consistent with preserved fricative and aspiration energy.

These activations align with the FP-SA frequency prior. The FP-SA module applies a learnable Gaussian bias centred on the 250--2000\,Hz sub-band (shaded in Figure~\ref{fig:gradcam_freqbands}), and Grad-CAM confirms this is where the model draws the boundary between PD-like and normal speech. Misclassified HC subjects activate this band at the same level as true PD subjects, which supports the pre-symptomatic hypothesis: the model is responding to spectral changes in the formant and voice-quality bands that are physiologically consistent with early PD dysarthria rather than to noise or artefact. An alternative-disorder explanation (e.g.\ functional dysphonia, age-related vocal fold changes) would produce the same activation pattern and cannot be excluded without clinical follow-up.

Figure~\ref{fig:gradcam_comparison} reinforces this: the heatmap overlays for misclassified HC subjects (columns 1--3) are visually indistinguishable from those of correctly classified PD subjects (columns 4--5) in the low-to-mid frequency bands, while correctly classified HC subjects (columns 6--7) show more diffuse, higher-frequency activation. The Fold~3 false positives are not an architectural failure but a genuine spectral ambiguity in the data, and the FP-SA prior is tuned to the band where that ambiguity sits.
...
The regulatory pathway depends on jurisdiction. In the UK, a clinical AI system for PD detection would be classified as a Class~IIa in vitro diagnostic device under UK MDR 2002 (as amended post-Brexit), requiring UKCA marking, a Quality Management System conformant with ISO~13485, and a clinical evaluation report. In the EU, the same system falls under the EU AI Act (2024) as a high-risk AI system (Annex~III, point~5a), requiring conformity assessment under MDR 2017/745 before CE marking is granted. The FDA 510(k) pathway in the US would require substantial equivalence to a predicate device, or a De Novo submission if no predicate exists. All three pathways require per-subgroup performance reporting. A known HC subgroup with a 25\% false positive rate would need explicit mitigation in the regulatory submission --- one approach would be a clinical flag requiring a standard neurological review before acting on a high-confidence PD prediction for any subject falling within the borderline spectral profile identified in Fold~3.
...
\begin{enumerate}
    ...
    \item \textbf{Extended Grad-CAM Analysis.} Section~\ref{sec:gradcam} presents Grad-CAM results for the ResNet-50 model; applying the same analysis to the full \dualcnn{} model once checkpoints are preserved would allow a direct comparison of attention patterns between the single-backbone and dual-branch architectures, and could confirm whether FP-SA shifts activation further into the 250--2000\,Hz prior band relative to CA alone.
...
This thesis presented a \dualcnn{} architecture for Parkinson's Disease speech detection, combining dual-branch CNNs (ResNet50 and EfficientNetV2-S) with a custom FrequencyPriorSelfAttention module, Coordinate Attention, and a BiLSTM temporal head, evaluated under speaker-independent five-fold cross-validation with certified zero-leakage guarantees. The four research questions posed in Chapter~1 are addressed in turn below.

\textbf{RQ1} asked whether a dual-branch architecture with frequency-aware attention could achieve state-of-the-art performance under speaker-independent CV with formal leakage certification. The system achieves F1\,=\,$0.964 \pm 0.048$ ($N\!=\!15$, Wilcoxon $p < 0.0001$) on Italian PD. The closest published figure (Aversano et al., F1\,=\,0.971) does not report its splitting strategy or leakage controls, so a direct comparison is not meaningful. Under a fully documented, reproducible protocol, this is the strongest result the author is aware of on this dataset. \textbf{RQ1: answered.}

\textbf{RQ2} asked how FP-SA compares against standard attention mechanisms as an isolated experimental variable. The ablation in Table~\ref{tab:ablation_results} shows CA contributes $+0.7\%$ when added to the ResNet50 baseline, and the combined step of adding the EfficientNetV2-S branch together with FP-SA contributes $+0.6\%$. Because FP-SA and CA are fused in parallel in the dual-branch model, their individual contributions cannot be read separately from this ablation; FP-SA in isolation was not tested as a standalone row. This is acknowledged as a limitation. What the data does show is that the full system, including FP-SA, outperforms every ablation variant, and the Grad-CAM analysis in Section~\ref{sec:gradcam} confirms that the model attends to precisely the frequency band the FP-SA prior targets. \textbf{RQ2: partially answered; FP-SA isolated ablation is flagged as future work.}

\textbf{RQ3} asked how well a model trained on Italian generalises to Colombian Spanish DDK tasks. Performance drops from F1\,=\,0.964 (Italian PD) to F1\,=\,0.780 (PC-GITA), a gap of 0.184. This quantifies for the first time the cross-lingual transfer cost for a spectrogram-based PD system under identical conditions. The gap reflects differences in both language and speech task (read speech vs.\ diadochokinesis), and neither factor can be isolated without additional experiments. \textbf{RQ3: answered.}

\textbf{RQ4} asked what fold-level instability reveals about clinical dataset composition. Fold~3 consistently shows PD recall of 1.000 and HC recall of approximately 0.75, across all architectures and all seeds. Inference on the Fold~3 validation set identifies 22 HC subjects predicted as PD-positive; Grad-CAM analysis shows their activation is concentrated in the same 250--2000\,Hz formant band as true PD subjects. This is consistent with either a pre-symptomatic PD subgroup or subjects with unrelated vocal disorders that produce overlapping spectral patterns. The subgroup is invisible under single-split evaluation and has not been previously characterised in the literature. \textbf{RQ4: answered.}

The work makes five contributions: the \dualcnn{} architecture with FP-SA; a three-level certified zero-leakage evaluation pipeline; the first unified cross-lingual PD detection evaluation across Italian and Colombian Spanish; characterisation of the Fold~3 borderline HC cluster; and a six-phase documented architectural progression where every design decision is grounded in experimental evidence. Both leakage bugs discovered during the project were caught and corrected before any final results were generated, and the Strategy~A$\to$B pivot corrected a confounded experimental design before conclusions were drawn from it.

The core findings have been submitted for publication as an IEEE conference paper covering the architecture, the evaluation protocol, and the cross-lingual results.
...
The generative AI tool was used solely for supportive purposes, including:
\begin{itemize}
    \item Clarifying technical concepts to aid my own understanding (e.g., convolutional neural networks, transfer learning, and attention mechanisms such as SE and CBAM).
    ...
    \item Generating illustrative system diagrams (including the data pipeline architecture, class structure diagrams, and training-loop sequence diagrams) based strictly on specifications, designs, and workflows defined by me. These diagrams were used solely for visual communication and presentation clarity. All underlying architectural decisions, system design choices, and implementations are entirely my own.
\end{itemize}

All model architectures, training pipelines, code implementations, experimental design, hyperparameter choices, evaluation procedures, results, and analysis were developed and written by me. Where external resources or publicly available implementations were consulted, they are explicitly acknowledged and referenced in the report in accordance with standard academic practice. No AI-generated text or content has been included verbatim without review, modification, and integration into my own work.
...
\bibitem{aversano2022} L.\ Aversano, M.\ L.\ Bernardi, M.\ Cimitile, R.\ Pecori, A.\ Picariello, G.\ Sperl\`{i}, and A.\ Testa, ``A machine learning approach for early detection of Parkinson's disease using acoustic traces,'' in \emph{Proc.\ IEEE EAIS}, 2022, pp.~1--8.
