...
AUG_SUFFIX = re.compile(r"_(orig|noisy|pitchUp\d+|stretch\d+(?:\.\d+)?)$", re.IGNORECASE)

def parse_args():
    ap = argparse.ArgumentParser()
    ap.add_argument("--spec_dir", required=True, help="Root dir containing PNG spectrograms")
    ap.add_argument("--esc50_csv", required=True, help="ESC-50 metadata CSV (has filename, category)")
    ap.add_argument("--out_dir", required=True, help="Where to write train/val CSVs")
    ap.add_argument("--val_ratio", type=float, default=0.2)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--outfile_prefix", default="", help="Optional prefix like 'clean_'")
...
def main():
    ...
    clip_tbl = df.groupby("clip_id").agg(label=("label", "first"), n_png=("filepath", "count")).reset_index()
    # Sanity: each clip_id should have a single label
    assert clip_tbl.groupby("clip_id")["label"].nunique().max() == 1, "Mixed labels within a clip_id."
    ...
    assert len(overlap) == 0, f"Leak persists for clip_ids: {list(sorted(overlap))[:10]}"

    # ---- Select output columns and write
    keep = ["filepath", "label"]
    prefix = args.outfile_prefix
    (out_dir / f"{prefix}train.csv").write_text(train_df[keep].sort_values("filepath").to_csv(index=False))
    (out_dir / f"{prefix}val.csv").write_text(val_df[keep].sort_values("filepath").to_csv(index=False))

    # ---- Report
    print(f"Total PNGs: {len(df)} | Clips: {len(clip_tbl)}")
    print(f"Train PNGs: {len(train_df)} | Clips: {len(train_clips)} | Classes: {train_df['label'].nunique()}")
    print(f"Val   PNGs: {len(val_df)} | Clips: {len(val_clips)} | Classes: {val_df['label'].nunique()}")
