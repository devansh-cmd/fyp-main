...
class DualCNNSALSTM(nn.Module):
    """
    EfficientNetV2-S + ResNet-50 dual-branch → FP-SA → BiLSTM → Classifier.
    Backbones are frozen initially and selectively unfrozen via unfreeze_backbones().
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        assert x.shape[-2:] == (224, 224), f"Expected 224x224 input, got {x.shape}"

        # EfficientNet branch → (B, 1280, 7, 7)
        eff_feat = self.effnet_pool(self.effnet_features(x))

        # ResNet branch → (B, 2048, 7, 7)
        ...
        # Fuse branches and project to 512 channels
        joint = torch.cat([eff_feat, res_feat], dim=1)  # (B, 3328, 7, 7)
        joint = self.joint_proj(joint)                   # (B, 512, 7, 7)

        # Frequency-Prior Self-Attention
        joint_sa = self.sa(joint) if self.use_sa else joint

        # Flatten spatial grid to sequence, run BiLSTM, mean-pool
        seq = joint_sa.flatten(2).permute(0, 2, 1)  # (B, 49, 512)
        out, _ = self.lstm(seq)
...
if __name__ == "__main__":
    ...
    print(f"output: {out.shape}  total params: {total:,}  trainable: {trainable:,}")
