...
def process_file_pitt_rigor(row, spec_out: Path, target_sr: int = 16000, window_sec: int = 20, overlap_sec: int = 5, is_train: bool = True) -> int:
    """
    ...
    try:
        ...
        for win, start_sec in zip(windows, starts):
            ...
            if is_train:
                ...
                S_db_noisy = compute_logmel(y_noisy, sr, n_fft=2048, hop_length=512, n_mels=128)
...
def main():
    ...
    SPEC_OUT = PROJECT_ROOT / "product" / "audio_preprocessing" / "outputs" / "spectrograms_pitt"
    ...
    for _, row in tqdm(df_train.iterrows(), total=len(df_train), desc="Generating Train Spectrograms"):
        total_segments += process_file_pitt_rigor(row, SPEC_OUT, is_train=True)
        
    # Process Val Set (Deterministic 1-output)
    for _, row in tqdm(df_val.iterrows(), total=len(df_val), desc="Generating Val Spectrograms"):
