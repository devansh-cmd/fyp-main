...
class SpectrogramCSVDataset(Dataset): # dataset(same as baseline)
    ...
    def __init__(self, csv_path: str, label2id: dict, img_size: int = 224):
        ...
        assert {"filepath", "label"}.issubset(self.df.columns), f"{csv_path} missing required columns!"
        self.label2id = label2id
        self.img_size = img_size

        missing = [p for p in self.df["filepath"] if not Path(p).exists()]
        if missing:
            raise FileNotFoundError(f"{len(missing)} missing files (first 5): {missing[:5]}")
...
def main(): # Actual main training function
    ...
    # loading pretrained ResNet-50 from torchvision (weights trained on Imagenet)
    ...
    in_feats = model.fc.in_features
    model.fc = nn.Linear(in_feats, num_classes)
    for p in model.layer4.parameters():# Unfreeze only last block + head
    ...
    with open(tb_logdir / f"resnet50_t1_metrics_e{args.epochs}.json", "w") as f:
        json.dump(history, f, indent=2)

    per_class_recall = {k: v["recall"] for k, v in rep.items() if isinstance(v, dict) and "recall" in v}
    summary = {
    ...
    with open(tb_logdir / f"resnet50_t1_summary_e{args.epochs}.json", "w") as f:
